...
sst_clean <- sst |> # create new object from sst
  select(date, temp) |> # select columns of interest
  mutate(date = as_date(date)) |> # make sure date is a date
  filter(date > as_date("2017-12-31")) |> 
  # filter to only includes dates from 2018 and after
  mutate_at(vars(date), list(year, month)) |>  # add month and year column based off of date
  rename(year = fn1, # rename fn1 column to year
         month = fn2) |> # rename fn2 column to month
  # make month have levels 1 through 12
  group_by(year, month) |> # group by month and year
  summarize(mean_monthly_sst = mean(temp, na.rm = TRUE)) |> # calculate monthly mean sst
