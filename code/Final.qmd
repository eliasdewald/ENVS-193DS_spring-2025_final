...
library(ggeffects) # additional ggplot features
...
nest_boxes_clean <- nest_boxes |>  # make new data frame from nest_boxes data frame
  mutate(season = as_factor(season), # make sure season is a factor
         season = fct_relevel(season, "2016", "2018")) # change factor levels
plot2 <- ggplot(data = nest_boxes, # make new object by plotting nest_boxes
       aes(x = edge_distance, # set x-axis
           y = sp, # set y-axis
           color = season)) + # color by season
  geom_point(width = 0.1, # jitter underlying data as points and set width
              height = 0, # set height
